...
extern const char* get_category_label(int index);
...
TfLiteStatus RecognizeCommands::ProcessLatestResults(const TfLiteTensor* latest_results,
    ...
    if ((!previous_results_.empty()) && (current_time_ms < previous_results_.front().time_)) {
        printf(
            ...
            "timestamp of %ld that was earlier than the previous one of %ld\n",
            current_time_ms,
            previous_results_.front().time_);
        return kTfLiteError;
    }
    ...
    // Convert the model output from int8 to uint8
        ...
        printf("Unsupported output tensor data type, must be int8 or float32\n");
        return kTfLiteError;
    }
    ...
    if (minimum_count_ == 0) {
        ...
        if ((how_many_results < minimum_count_)) {
            ...
            if (consecutive_min_count % 10 == 0) {
                printf(
                    "Too few samples for averaging. This likely means the inference loop is taking too long.\n");
                printf("Try one of the following:\n");
                printf("- Ensure the app was built for \"release\"\n");
                printf("- Ensure the HW accelerator is enabled\n");
                printf(
                    "- Decrease the 'minimum_count' and/or increase 'average_window_duration_ms' model parameters\n");
                printf("- Use a smaller ML model\n");
            }
    ...
    if ((current_top_score > detection_threshold) && (ignore_underscore_ && current_top_label[0] != '_') &&
        ((current_top_index != previous_top_label_index_) || (time_since_last_top > suppression_ms_))) {
