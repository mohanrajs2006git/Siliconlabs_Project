...
TfLiteStatus mvp_prepare(TfLiteContext* context, TfLiteNode* node)
{
    ...
    const auto padding = ComputePaddingHeightWidth(
            ...
            input_height,  input_width, filter_height, filter_width, params->padding,
    ...
    op_params.padding_type = runtime_padding_type(params->padding);
    ...
    if(npu_toolkit::mvpv1::conv2d::is_supported(
        context,
        op_params,
        input_shape,
        filter_shape,
...
{
    auto& op_params     = (ConvParams&)data.op_params;
    ...
    {
        auto scaled_input_buffer = NPU_TOOLKIT_GET_SCRATCH_BUFFER(float16_t, data.scratch_buffer_index);
         return npu_toolkit::mvpv1::conv2d::calculate(
            context,
            op_params,
            data.per_channel_output_scaler,
            scaled_input_buffer,
    ...
    if(data.kernel_type == KernelType::Compiled)
    {
        ...
        return npu_toolkit::mvpv1::conv2d::calculate_compiled(
            context,
            input_data,
            scaled_input_buffer,
            filters_data,
