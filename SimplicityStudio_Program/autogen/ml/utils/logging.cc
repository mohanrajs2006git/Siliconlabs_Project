...
Logger::Logger(const char *tag, Level level, Flags flags) :
    _level(level), _flags(flags), _writer(default_log_writer)
{
...
void Logger::vwrite(Level level, const char *fmt, va_list args) const
{
    if ((_writer != nullptr) && (_level != Level::Disabled) && (level >= _level))
    {
        ...
        {
            ptr += length;
            if (_flags.is_set(Flag::Newline))
            {
                *ptr++ = '\n';
            }

            const int len = (int)(ptr - fmt_buffer);
            if(len > 0)
            {
                _writer(fmt_buffer, len, _writer_arg);
            }
        }
    }
}
...
int Logger::write_prefix(Level level, char *buffer, unsigned buffer_length) const
{
    ...
    if (_flags.is_set(Flag::PrintLevel))
    {
    ...
    if (_flags.is_set(Flag::PrintTag))
    {
