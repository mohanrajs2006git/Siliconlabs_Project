...
#define DEFINE_ENUM_CLASS_BITMASK_OPERATORS(E, T)\
    __STATIC_FORCEINLINE E operator | (E lhs, E rhs){\
    ...
    __STATIC_FORCEINLINE E operator ~ (E lhs){\
        return E(~T(lhs));\
    }\
    __STATIC_FORCEINLINE E operator |= (E &lhs, E rhs){\
        lhs = lhs | rhs;\
    ...
    __STATIC_FORCEINLINE bool operator !(E rhs) \
    {\
        return !bool(T(rhs)); \
    }

#define DEFINE_ENUM_CLASS_OPERATORS(E, T)\
    DEFINE_ENUM_CLASS_BITMASK_OPERATORS(E, T) \
    __STATIC_FORCEINLINE bool  operator > (E lhs, E rhs){\
        return T(lhs) > T(rhs);\
    }\
    __STATIC_FORCEINLINE bool operator > (E lhs, int rhs){\
        return T(lhs) > rhs;\
    }\
    __STATIC_FORCEINLINE bool operator > (int lhs, E rhs){\
        return lhs > T(rhs);\
    }\
    __STATIC_FORCEINLINE bool operator < (E lhs, E rhs){\
        return T(lhs) < T(rhs);\
...
__STATIC_FORCEINLINE unsigned int align_up(int value, unsigned align_by)
{
    return ((value + align_by - 1) / align_by) * align_by;
}

__STATIC_FORCEINLINE unsigned int align_down(int value, unsigned divide_by)
{
...
#if defined(NPU_TOOLKIT_DLL_EXPORT) && !defined(DLL_EXPORT)
#  if defined(WIN32) || defined(_WIN32)
#    define DLL_EXPORT(x) __declspec(dllexport) x
#  else
#    define DLL_EXPORT(x) x
#  endif
#endif
